...
def predict_tc99m_yield(arrival_date, initial_activity):
    ...
    half_life_Mo99=RADIOACTIVE_NUCLIDES["Mollebdenium (MO-99)"]*24
...
def authenticate(password):
    if password == CORRECT_PASSWORD:
        success_message = gr.Markdown("<p style='color:green; font-weight:bold;'>✅ Access Granted!</p>")
        return success_message, gr.Textbox(visible=True), gr.Button(visible=True)
        
    else:
        failure_message = gr.Markdown("<p style='color:green; font-weight:bold;'> Access Denied!Incorrect Password!</p>") 
...
with gr.Blocks() as ui:
    ...
    gr.Markdown("""
        <style>
            body {
                background-color: white !important; /* Set background to black */
            }
            .gradio-container {
                background-color: white !important; /* Set container background */
                color: white !important; /* Default text color */
                font-family: Arial, sans-serif;
            }
            h1 {
                color: black !important; /* White heading color */
    ...
    gr.Markdown("<h1 style='text-align: center;margin-bottom:0px; color: blue;'>**MedicalPhysics-Nexus**</h1>")#, unsafe_allow_html=True)
    ...
    # Footer text
    gr.Markdown("<h3 style='text-align: right;font-size: 7.0px ; color: blue;'>SDeveloped by Medical Physics Division(AEMCK)</h3>")#, unsafe_allow_html=True)

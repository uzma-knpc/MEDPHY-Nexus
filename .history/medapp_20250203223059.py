...
def medical_physics_agent(query):
    ...
    if "convert" in query:
        ...
        title = "Occupational Dose Assessment"
            ...
            Result= Pridicted Yeild for next 7 days
            title="Pridicted Yeild for Mo-99"
    else:
...
tools=[predict_tc99m_yield,print_exposure_limits,patient_release_decision,unit_conversion_tool,radioactive_decay_tool]
...
with gr.Blocks() as ui:
    ...
    gr.Markdown("<p>An intelligent system for unit conversions, decay calculations, and the management of Iodine-treated patient releases. Ensures compliance with radiation protection regulations and medical physics standards, integrating all relevant safety limits and guidelines.</p>")#, unsafe_allow_html=True)
